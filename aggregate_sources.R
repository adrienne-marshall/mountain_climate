...
keep_titles <- read_csv("data/titles_to_keep.csv")
...
keep_titles <- keep_titles %>% select(-include1, -include2, -source)

#filter to titles in keep_titles. 
dat <- dat %>%
  dplyr::select(-include_final) %>%
  right_join(keep_titles, by = "title")

write_csv(dat, "data/aggregated_data.csv")
