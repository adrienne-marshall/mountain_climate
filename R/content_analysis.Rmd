...
```{r get data, echo = F}
dat <- read.csv("../results/tabular/content_coding.csv",
                stringsAsFactors = F)
names(dat) <- c("time", "name", "title", "paper_id", "I_A_M", "impact_type", "resolution", "extent", "location", "biome", "huc6", "data_type", "discipline", "topic", "concerns", "content1", "content2")

# Get only papers that have been duplicated. 
duplicated <- dat %>% 
  group_by(paper_id) %>%
  count(sort = T) %>%
  filter(n > 1)
...
dat$name[dat$name == "Shana"] <- "SH"

dat <- dat %>% filter(paper_id %in% duplicated$paper_id)
...
```
 ...
 `df1` is a dataframe of each of the papers coded by one person, and `df2` is the same data frame, with papers coded by the other person. Do we have any apparent systematic bias in who is showing up as a coder in each one? 
...
```{r impact_types}
ggplot(dat, aes(x = impact_type, fill = set)) + 
  geom_bar(position = "dodge") + coord_flip() 
```

That looks pretty good. I think we'll ultimately want to separate this so that we're just counting the individual categories (e.g., "projected impacts; observed impacts" will get counted in both those categories but not together), but this is fine for now. 
...
Let's start with locations that have a numeric answer. For now, I took out anything that also words in it, even if it has points too. Let's also subset this to a reasonable spatial extent - maybe less than 500 km^2??

There were also a few points that were way out at seas... so I assume those are mistakes, and got rid of them. 
...
ggplot(biome_dat, aes(x = biome_list, y = n, fill = set)) + 
  geom_col(position = "dodge") + coord_flip() 
```
...
ggplot(huc_dat, aes(x = huc6, y = n, fill = set)) + 
  geom_col(position = "dodge") + coord_flip() 
```

Agh, that one kind of makes my eyes hurt... and it looks like there is potentially a difference between the sets, particularly for the Canadian basins. 
...
ggplot(data_dat, aes(x = data_type, y = n, fill = set)) + 
  geom_col(position = "dodge") + coord_flip() 
```
...
ggplot(discipline_dat, aes(x = discipline, y = n, fill = set)) + 
  geom_col(position = "dodge") + coord_flip() 
